...
AGGREGATOR_ENABLED=false  # Default value; set to true to enable aggregator mode
...
# Function to restart service based on aggregator mode
restart_service() {
...
# Restart services based on aggregator mode
if [ "$AGGREGATOR_ENABLED" = "true" ]; then
    sudo systemctl is-active --quiet wfb-cluster-node || sudo systemctl restart wfb-cluster-node
else
            ...
            if [ "$DVR_BUTTON_STATE" -eq 1 ]; then
                if [ "$DVR_RECORDING" -eq 0 ]; then
                    ...
                    if [ -n "$latest_video" ]; then
                        ...
                        # Generate thumbnail
                        ffmpegthumbnailer -i "$latest_video" -o "/etc/webUI/static/thumbnails/${filename}.jpg" -s 320
                    fi
                fi
